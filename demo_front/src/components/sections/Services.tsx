...
const Item = ({ title, delay, icon: Icon, isActive }: ItemProps) => (
  <motion.div
    initial={{ opacity: 0, y: 10 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true }}
    transition={{ delay, duration: 0.4, ease: "easeOut" }}
    className={`flex items-center gap-4 p-4 rounded-2xl transition-all duration-500 border ${isActive
        ? 'bg-white shadow-lg shadow-brand-primary/10 border-brand-primary/30 scale-[1.02]'
        : 'bg-brand-bg/50 border-transparent'
      } cursor-default`}
  >
    <div className={`p-2.5 rounded-xl transition-all duration-500 ${isActive ? 'bg-brand-primary text-white scale-110 shadow-lg shadow-brand-primary/30' : 'bg-brand-primary/10 text-brand-primary'
      }`}>
      {Icon ? <Icon size={20} /> : <CheckCircle2 size={20} />}
    </div>
    <span className={`font-bold text-base leading-tight transition-colors duration-500 ${isActive ? 'text-brand-primary' : 'text-brand-text/70'
      }`}>{title}</span>
  </motion.div>
);
